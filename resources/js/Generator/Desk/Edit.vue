...
const form = useForm({
    name: props.desk.data.name,
    directory: props.desk.data.directory,
    child_table: props.desk.data.child_table,
    parent_table: props.desk.data.parent_table,

    has_filter: props.desk.data.has_filter,
    has_opening: props.desk.data.has_opening,
    columns_in_row: props.desk.data.columns_in_row,
    has_polymorphic: props.desk.data.has_polymorphic,
    has_description: props.desk.data.has_description,
    has_remark: props.desk.data.has_remark,
    has_has_soft_deletes: props.desk.data.has_has_soft_deletes,

    group_pillars: props.desk.data.group_pillars.length? props.desk.data.group_pillars : [{
        title: null,
        table_id: null,
        column: null,
        unique: false,
...
const submit = () => {
    form.post(route('generator.desk.update', props.desk.data.id), {
        onFinish: () => {
        }
    });
}
    ...
    <div class="max-w-full mx-auto py-5 sm:px-6 lg:px-8">
        <div class="bg-white shadow sm:rounded-lg">
            <div class="flex justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
                <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium"> {{ desk.data.name }} Desk Edit</p>

                <div class="flex-shrink-0 flex space-x-3">
                    <Link :href="route('generator.desk.decorate', desk.data.id)" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-500 hover:text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-gray-500">
                    ...
                    <Link :href="route('generator.desk.generate_files', desk.data.id)" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-500 hover:text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-gray-500">
                ...
                <dl class="space-y-8 py-6">
                    <div class="max-w-6xl mx-auto">
                        <div class="py-2 sm:grid sm:grid-cols-4 sm:gap-2">
                            ...
                            <dd class="flex mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 gap-5">
                                <label class="my-2 inline-flex items-center">
                                    <input v-model="form.columns_in_row" value="2" type="radio" class="form-radio w-5 h-5 border-gray-500 text-gray-500 cursor-pointer focus:ring-gray-500">
                                    <span class="ml-2 tracking-wide cursor-pointer"> 2 in a Row</span>
                                </label><br>
                                <label class="my-2 inline-flex items-center">
                                    <input v-model="form.columns_in_row" value="3" type="radio" class="form-radio w-5 h-5 border-gray-500 text-gray-500 cursor-pointer focus:ring-gray-500">
                        ...
                        <div class="py-2 sm:grid sm:grid-cols-4 sm:gap-2">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                <input v-model="form.parent_table" placeholder="Parent Table" type="text" class="block w-full px-4 focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded">
                            </dd>

                            <dt class="text-sm leading-10 font-semibold text-gray-700 tracking-wider text-right pr-8"> Child Table </dt>
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                <input v-model="form.child_table" placeholder="Child Table" type="text" class="block w-full px-4 focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded">
                            </dd>
                        </div>
                        ...
                        <div class="py-2 sm:grid sm:grid-cols-4 sm:gap-2">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                <input v-model="form.directory" placeholder="Directory" type="text" class="block w-full px-4 focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded">
                            </dd>
                        </div>
                        ...
                        <div class="py-2 sm:grid sm:grid-cols-4 sm:gap-2">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                <div class="mt-1 inline-flex items-center py-2 text-sm font-medium rounded text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600">
                                    <input v-model="form.has_polymorphic" type="checkbox" class="group-checkbox form-checkbox w-5 h-5 focus:ring-gray-600 focus:border-gray-600 text-gray-600 transition duration-150 ease-in-out rounded select-none cursor-pointer">
                        ...
                        <table class="table-auto sm:table-fixed min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="(group_pillar, index) in form.group_pillars" :key="index">
                                    ...
                                    <!-- <td>
                                        <input v-model="group_pillar.column" readonly placeholder="Column" type="text" class="block w-full px-2 focus:ring-none focus:ring-0 focus:ring-gray-400 focus:border-gray-400 bg-gray-100 sm:text-sm border-gray-300 rounded" />
                                    </td> -->

                                    <td>
                                        <input v-model="group_pillar.filtering" placeholder="Filtering" type="text" class="block w-full px-2 focus:ring-none focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded" />
                                    </td>

                                    <td>
                                        <input v-model="group_pillar.indexing" placeholder="Indexing" type="text" class="block w-full px-2 focus:ring-none focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded" />
                                    </td>

                                    <td>
                                        <div class="inline-flex items-center py-2 text-sm font-medium rounded text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600">
                                            <input v-model="group_pillar.unique" type="checkbox" class="ml-6 group-checkbox form-checkbox w-5 h-5 focus:ring-gray-600 focus:border-gray-600 text-gray-600 transition duration-150 ease-in-out rounded select-none cursor-pointer" />
                                        </div>
                                    </td>

                                    <td>
                                        <input v-model="group_pillar.default" placeholder="Default" type="text" class="block w-full px-2 focus:ring-none focus:ring-gray-400 focus:border-gray-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded" />
                                    </td>

                                    <td>
                                        <div class="inline-flex items-center py-2 text-sm font-medium rounded text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600">
                                            <input v-model="group_pillar.requisite" type="checkbox" class="mx-2 group-checkbox form-checkbox w-5 h-5 focus:ring-gray-600 focus:border-gray-600 text-gray-600 transition duration-150 ease-in-out rounded select-none cursor-pointer" />
                            ...
                            <tfoot class="bg-white">
                                <tr>
                                    <th colspan="1" class="text-left pt-3">
                                        <button @click="addPillar()" type="button" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                                            Add Pillar
